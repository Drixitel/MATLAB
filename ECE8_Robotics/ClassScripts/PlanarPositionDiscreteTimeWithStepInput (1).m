...
% define vx and vy such that the position of the robot ends at
pxd = 1.6; % desired x position
pyd = 0.5; % desired y position

% vx defined as a step signal that is equal to 1
vx = ones(1,N);

k = 0;
% define vy so that is equal to 1 until k = 49, and after that is 0
for i = 1:N
    if (i < 51) % condition for if would be true if i < 51
        vy(i) = 1;
    else
        vy(i) = 0;        
    end
